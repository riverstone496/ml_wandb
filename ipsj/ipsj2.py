...
if __name__=='__main__':
    ...
    save_dict=False
    load_dict=True
    ylabel_name='Error rate'
    metric='test_accuracy'
    folder = './ipsj/graph/'
    filename=folder+'ipsj2.png'
    sweep_list_mlp={
    ...
    if save_dict:
        ...
        with open("./pickle/clip_test_dic_mlp.pickle", "wb") as tf:
            pickle.dump(clip_test_dic_mlp,tf)
        with open("./pickle/clip_test_dic_res.pickle", "wb") as tf:
            pickle.dump(clip_test_dic_res,tf)
        with open("./pickle/clip_test_dic_vit.pickle", "wb") as tf:
            pickle.dump(clip_test_dic_vit,tf)

    if load_dict:
        with open("./pickle/clip_test_dic_mlp.pickle", "rb") as tf:
            clip_test_dic_mlp = pickle.load(tf)
        with open("./pickle/clip_test_dic_res.pickle", "rb") as tf:
            clip_test_dic_res = pickle.load(tf)
        with open("./pickle/clip_test_dic_vit.pickle", "rb") as tf:
            clip_test_dic_vit = pickle.load(tf)
    ...
    for j in range(len(optim_list)):
        ...
        axes[0].set_ylabel('MLP '+ylabel_name)
        axes[1].set_ylabel('Resnet18 '+ylabel_name)
        axes[2].set_ylabel('ViT-T '+ylabel_name)
